...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.wordGroupService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<WordGroup[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/word-group'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/word-group', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: WordGroup) {
        return item.id;
    }
    registerChangeInWordGroups() {
        this.eventSubscriber = this.eventManager.subscribe('wordGroupListModification', (response) => this.loadAll());
    }
